...
| `tracker.required_labels`, active states, terminal states | Runtime wired | Required labels and active states gate provider intake; durable reconciliation terminates and releases terminal or ineligible runs. State lists accept only `open`, `in_progress`, `review`, `done`, and `cancelled`. |
...
| P3 Linear adapter | Implemented and live-canary tested | GraphQL adapter and intake routing tests cover authentication, project pagination, issue normalization, provider routing, and fail-closed configuration. A private project canary verified read-only credentials, single claim, restart recovery, active-state progression, dynamic reload, and terminal release. |
